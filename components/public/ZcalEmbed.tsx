...
export function ZcalEmbed({ isDark = false }: { isDark?: boolean }) {
  ...
  const themeClasses = isDark 
    ? "bg-[#132A4E]/30 border border-[#2563EB]/25 shadow-xl" 
    : "bg-white border border-[#E2E8F0] shadow-sm";

  const textHeading = isDark ? "text-white" : "text-[#1E293B]";
  const textSub = isDark ? "text-[#aebcda]" : "text-[#475569]";
  const borderClass = isDark ? "border-white/10" : "border-[#E2E8F0]";
  ...
  return (
    <div className={`w-full rounded-[28px] p-4 sm:p-6 lg:p-8 transition-all ${themeClasses}`}>
      ...
      <div className={`flex flex-wrap items-center justify-between gap-3 pb-5 mb-5 border-b ${borderClass}`}>
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-xl grid place-items-center shrink-0 ${
            isDark ? 'bg-[#2563EB]/20 border border-[#2563EB]/30 text-[#4c9bff]' : 'bg-[#2563EB]/10 border border-[#2563EB]/20 text-[#2563EB]'
          }`}>
            <Calendar size={20} />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="flex h-2 w-2 relative">
                <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isDark ? 'bg-[#25D366]' : 'bg-[#15803D]'}`}></span>
                <span className={`relative inline-flex rounded-full h-2 w-2 ${isDark ? 'bg-[#25D366]' : 'bg-[#15803D]'}`}></span>
              </span>
              <span className={`text-[11px] font-extrabold uppercase tracking-widest ${isDark ? 'text-[#25D366]' : 'text-[#15803D]'}`}>Live Booking Active</span>
            </div>
        ...
        <div className="flex items-center gap-4 text-[12.5px] font-medium">
          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg border ${
            isDark ? 'bg-[#050f1f]/50 border-white/5 text-[#aebcda]' : 'bg-[#F8FAFC] border-[#E2E8F0] text-[#475569]'
          }`}>
            <Clock size={14} className={isDark ? 'text-[#4c9bff]' : 'text-[#2563EB]'} />
            15 Minutes
          </span>
          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg border ${
            isDark ? 'bg-[#050f1f]/50 border-white/5 text-[#aebcda]' : 'bg-[#F8FAFC] border-[#E2E8F0] text-[#475569]'
          }`}>
            <ShieldCheck size={14} className={isDark ? 'text-[#25D366]' : 'text-[#15803D]'} />
      ...
      {!mounted ? (
        <div className={`w-full flex flex-col items-center justify-center text-center min-h-[600px] rounded-2xl border p-8 ${
          isDark ? 'bg-[#050f1f]/30 border-white/5' : 'bg-[#F8FAFC] border-[#E2E8F0]'
        }`}>
        ...
        <div className={`zcal-inline-widget w-full min-h-[700px] rounded-2xl overflow-hidden ${isDark ? 'bg-[#050f1f]/40' : 'bg-[#F8FAFC]'}`}>
          <iframe 
            src={zcalLink} 
